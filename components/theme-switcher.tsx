...
export function ThemeSwitcher() {
  ...
  if (!mounted) {
    return (
      <div className="relative z-0 inline-grid grid-cols-3 gap-0.5 rounded-full bg-gray-950/5 p-0.75 text-gray-950 dark:bg-white/10 dark:text-white h-9 w-[100px] opacity-0" />
    )
  }

  const btnClass =
    "flex items-center justify-center rounded-full p-1.5 [&>svg]:size-7 sm:[&>svg]:size-5 sm:p-1 data-checked:bg-white data-checked:ring-1 data-checked:shadow-sm data-checked:ring-gray-950/10 dark:data-checked:bg-gray-700 dark:data-checked:text-white dark:data-checked:ring-transparent transition-colors hover:text-gray-900 dark:hover:text-white text-gray-500 dark:text-gray-400 data-checked:text-gray-950"
